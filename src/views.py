...
class StartView(arcade.View):
    ...
    def on_draw(self) -> bool | None:
        ...
        start_view = arcade.Text("Press start", self.window.width / 2, self.window.height / 2,
                         arcade.color.WHITE, font_size=50, anchor_x="center")
        start_view.draw()

class GameView(arcade.View):
    def __init__(self, window: arcade.Window, game_config: GameConfig, start_view: StartView):
        super().__init__()
...
class GameOverView(arcade.View):
    ...
    def on_draw(self) -> bool | None:
        game_over_text = arcade.Text("Game Over Screen", self.window.width / 2, self.window.height / 2 + 100,
                         arcade.color.WHITE, font_size=20, anchor_x="center")
        game_over_text.draw()

        msg = arcade.Text("Press space to go to the meny", self.window.width / 2, self.window.height / 2 - 100,
